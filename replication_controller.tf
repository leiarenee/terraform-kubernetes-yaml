  ...
  dynamic "spec" { # Nesting Mode: list  Min Items : 1  Max Items : 1  
    ...
    content {
      ...
      dynamic "template" { # Nesting Mode: list  Min Items : 1  Max Items : 1  
        ...
        content {
          active_deadline_seconds = lookup(template.value, "activeDeadlineSeconds", null)
          # Type: number   Optional Computed 
          # Optional duration in seconds the pod may be active on the node relative to StartTime before the system will actively try to mark it failed and kill associated containers. Value must be a positive integer.

          automount_service_account_token = lookup(template.value, "automountServiceAccountToken", null)
          # Type: bool   Optional  
          # AutomountServiceAccountToken indicates whether a service account token should be automatically mounted.

          dns_policy = lookup(template.value, "dnsPolicy", null)
          # Type: string   Optional Computed 
          # Set DNS policy for containers within the pod. Valid values are 'ClusterFirstWithHostNet', 'ClusterFirst', 'Default' or 'None'. DNS parameters given in DNSConfig will be merged with the policy selected with DNSPolicy. To have DNS options set along with hostNetwork, you have to specify DNS policy explicitly to 'ClusterFirstWithHostNet'. Optional: Defaults to 'ClusterFirst', see [Kubernetes reference](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-policy).

          enable_service_links = lookup(template.value, "enableServiceLinks", null)
          # Type: bool   Optional  
          # Enables generating environment variables for service discovery. Optional: Defaults to true.

          host_ipc = lookup(template.value, "hostIPC", null)
          # Type: bool   Optional Computed 
          # Use the host's ipc namespace. Optional: Defaults to false.

          host_network = lookup(template.value, "hostNetwork", null)
          # Type: bool   Optional Computed 
          # Host networking requested for this pod. Use the host's network namespace. If this option is set, the ports that will be used must be specified.

          host_pid = lookup(template.value, "hostPid", null)
          # Type: bool   Optional Computed 
          # Use the host's pid namespace.

          hostname = lookup(template.value, "hostname", null)
          # Type: string   Optional Computed 
          # Specifies the hostname of the Pod If not specified, the pod's hostname will be set to a system-defined value.

          node_name = lookup(template.value, "nodeName", null)
          # Type: string   Optional Computed 
          # NodeName is a request to schedule this pod onto a specific node. If it is non-empty, the scheduler simply schedules this pod onto that node, assuming that it fits resource requirements.

          node_selector = lookup(template.value, "nodeSelector", null)
          ...
          priority_class_name = lookup(template.value, "priorityClassName", null)
          # Type: string   Optional Computed 
          # If specified, indicates the pod's priority. "system-node-critical" and "system-cluster-critical" are two special keywords which indicate the highest priorities with the former being the highest priority. Any other name must be defined by creating a PriorityClass object with that name. If not specified, the pod priority will be default or zero if there is no default.

          restart_policy = lookup(template.value, "restartPolicy", null)
          # Type: string   Optional Computed 
          # Restart policy for all containers within the pod. One of Always, OnFailure, Never. More info: http://kubernetes.io/docs/user-guide/pod-states#restartpolicy.

          service_account_name = lookup(template.value, "serviceAccountName", null)
          # Type: string   Optional Computed 
          # ServiceAccountName is the name of the ServiceAccount to use to run this pod. More info: http://releases.k8s.io/HEAD/docs/design/service_accounts.md.

          share_process_namespace = lookup(template.value, "shareProcessNamespace", null)
          # Type: bool   Optional  
          # Share a single process namespace between all of the containers in a pod. When this is set containers will be able to view and signal processes from other containers in the same pod, and the first process in each container will not be assigned PID 1. HostPID and ShareProcessNamespace cannot both be set. Optional: Defaults to false.

          subdomain = lookup(template.value, "subdomain", null)
          # Type: string   Optional Computed 
          # If specified, the fully qualified Pod hostname will be "...svc.". If not specified, the pod will not have a domainname at all..

          termination_grace_period_seconds = lookup(template.value, "terminationGracePeriodSeconds", null)
          # Type: number   Optional Computed 
          ...
          dynamic "affinity" { # Nesting Mode: list  Max Items : 1  
            for_each = contains(keys(template.value), "affinity") ? {item = template.value["affinity"]} : {}
          ...
          dynamic "container" { # Nesting Mode: list  
            for_each = lookup(template.value, "containers", {})
          ...
          dynamic "dns_config" { # Nesting Mode: list  Max Items : 1  
            for_each = contains(keys(template.value), "dnsConfig") ? {item = template.value["dnsConfig"]} : {}
          ...
          dynamic "host_aliases" { # Nesting Mode: list  
            for_each = lookup(template.value, "hostAliases", {})
          ...
          dynamic "image_pull_secrets" { # Nesting Mode: list  
            for_each = lookup(template.value, "imagePullSecrets", {})
          ...
          dynamic "init_container" { # Nesting Mode: list  
            for_each = lookup(template.value, "initContainers", {})
          ...
          dynamic "metadata" { # Nesting Mode: list  Max Items : 1  
          ...
          dynamic "readiness_gate" { # Nesting Mode: list  
            for_each = lookup(template.value, "readinessGate", {})
          ...
          dynamic "security_context" { # Nesting Mode: list  Max Items : 1  
            for_each = contains(keys(template.value), "securityContext") ? {item = template.value["securityContext"]} : {}
          ...
          dynamic "toleration" { # Nesting Mode: list  
            for_each = lookup(template.value, "tolerations", {})
          ...
          dynamic "volume" { # Nesting Mode: list  
            for_each = lookup(template.value, "volumes", {})
